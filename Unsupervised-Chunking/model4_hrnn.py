...
from torchtext.legacy.data import Field, Iterator, BucketIterator
...
class HRNN(nn.Module):
    def __init__(self, input_size, hidden_size, vocab, output_size, batch_size, device):
        super(HRNN, self).__init__()
        self.device = device
        self.hidden_size = hidden_size

        self.rnn_lower_cut = nn.RNNCell(input_size, hidden_size)
        self.rnn_lower_nocut = nn.RNNCell(input_size, hidden_size)
        self.rnn_upper_cut = nn.RNNCell(hidden_size, hidden_size)

        self.hidden2tag = nn.Linear(hidden_size+hidden_size+input_size, output_size - 1).to(self.device)
        self.output_size = output_size
        self.batch_size = batch_size
        self.soft = nn.Softmax(dim=1)

    def forward(self, sentence, h_init, bert_sent, seqlens, max_seq_len):
        output_seq = torch.zeros((seqlens, self.output_size - 1)).to(self.device)

        h_lower_cut = h_init.to(self.device)
        h_lower_nocut = h_init.to(self.device)
        h_lower_actual = h_init.to(self.device)

        h_upper_cut = h_init.to(self.device)
        h_upper_nocut = h_init.to(self.device)
        h_upper_actual = h_init.to(self.device)
        ...
        for t in range(seqlens):
            ...
            h_lower_nocut = self.rnn_lower_nocut(entry, h_lower_actual)
            h_lower_cut = self.rnn_lower_cut(entry, h_init)
            
            h_upper_nocut = h_upper_actual
            h_upper_cut = self.rnn_upper_cut(h_lower_actual, h_upper_actual)
            
            h_lower_actual = torch.mul(h_lower_cut, mask_ind[0]) + torch.mul(h_lower_nocut, mask_ind[1])
            h_upper_actual = torch.mul(h_upper_nocut, mask_ind[0]) + torch.mul(h_upper_cut, mask_ind[1])

            if t == 0:
                h_init = h_lower_actual

            tag_rep = self.hidden2tag(torch.cat((h_lower_actual, h_upper_actual, next_entry), dim=1)).to(self.device)
            output = torch.squeeze(self.soft(tag_rep))
            output_seq[t] = output
        
        return output_seq, h_upper_actual
...
def main():
	...
	if args.is_training:
		...
		plt.plot(np.array(train_loss_vec).reshape([len(train_loss_vec), 1]), color="blue")
		plt.savefig(args.dire + 'train_loss_vec.png')
		plt.figure()
		plt.xlabel('number of epochs')
		plt.ylabel('fscore')
		plt.plot(np.array(val_fscore_vec).reshape([len(val_fscore_vec), 1]), color="red")
		plt.savefig(args.dire + 'val_fscore_vec.png')
